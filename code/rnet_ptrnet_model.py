...
class RNetPtrModel(QAModel):
    def build_graph(self):
        ...
        with tf.variable_scope('C2QAttention'):
            ...
            c2q_attn_dist, c2q_attention = c2q_attn_layer.build_graph(question_hiddens, self.qn_mask, context_hiddens) # (batch_size, context_len, hidden_size*4)
            # c2q_attn_dist, c2q_attention = c2q_attn_layer.build_mult_graph(question_hiddens, self.qn_mask, context_hiddens, self.FLAGS) # (batch_size, context_len, hidden_size*4)
            encoder = RNNEncoder(self.FLAGS.hidden_size, self.keep_prob)
